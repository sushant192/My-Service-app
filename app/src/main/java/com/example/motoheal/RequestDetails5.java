...
public class RequestDetails5 extends AppCompatActivity {

    ImageView imageView,image1,image2,image3,image4,image5,image6;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_request_details5);
        ...
        firebaseDatabase.getReference().child("Users").child(id).child("Requests").child(requestId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (snapshot.hasChild("Images")){
                    ...
                    if (snapshot.child("Images").hasChild("DesignImage1")){
                        String room4=snapshot.child("Images").child("DesignImage1").getValue().toString();
                        Picasso.get().load(room4).placeholder(R.drawable.ic_baseline_image_24).into(image4);
                    }
                    if (snapshot.child("Images").hasChild("DesignImage2")){
                        String room5=snapshot.child("Images").child("DesignImage2").getValue().toString();
                        Picasso.get().load(room5).placeholder(R.drawable.ic_baseline_image_24).into(image5);
                    }
                    if (snapshot.child("Images").hasChild("DesignImage3")){
                        String room6=snapshot.child("Images").child("DesignImage3").getValue().toString();
                        Picasso.get().load(room6).placeholder(R.drawable.ic_baseline_image_24).into(image6);
                    }
                }
            }
        ...
        firebaseDatabase.getReference().child("Prices").child("Painting").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
        ...
        accept.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent i=new Intent(RequestDetails5.this,PartnerMapActivity.class);
                startActivity(i);
                finish();
            }
        });
    ...
    private void makePhoneCall() {
        ...
        if (number.trim().length() > 0) {
            if (ContextCompat.checkSelfPermission(RequestDetails5.this,
                    Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(RequestDetails5.this,
            ...
            Toast.makeText(RequestDetails5.this, "Failed", Toast.LENGTH_SHORT).show();
        }
    }
